...
ParetoAnalysis <- function(inputValues = NULL, inputWeights = NULL, inputid = NULL, inputDemo = NULL, inputDemoName = NULL, x_0 = NULL, type, trun = "Untruncated", method = "RanDraw", simnum=1000, pre.specification = NULL, pre.top.trunc = NULL, graphs = FALSE, ksp = 0.01, ttp = 0.1, upperT = 0.85, lowerT=0.15, sim.ALPHA = 1.8, sim.SIGMA = 1.2, loopmax = 2000, calibrate.ALPHA=NULL, calibrate.SIGMA=NULL, HillEstimator=NULL, OptimSearch="Default", seeded=NULL, Sampling="Deterministic", model.selection = c("Arms", "Loglikelihood", "AIC", "BIC"), simulatePopulation = TRUE) {
  ...
  Pt1Sandwich <- SandwichMaker(simu$Value, simu$Weights, expression(w*(log(alpha)-log(x)+alpha*(log(gamma.pt1.hat2)-log(x)))), ParamNames=c("alpha"),ParamEstimates=c(alpha.pt1.hat2))
  ...
  GepSandwich <- SandwichMaker(simu$Value, simu$Weights, expression(w*(log(1/sigma)-((1+alpha)/alpha)*log((sigma+alpha*(x-gamma.gep.hat1))/sigma))), ParamNames=c("alpha", "sigma"),ParamEstimates=c(alpha.gep.hat1,sigma.gep.hat1))
  ...
  Tp1Sandwich <- SandwichMaker(simu$Value, simu$Weights, expression(w*(log(alpha)-log(x)+alpha*(log(gamma.pt1.hat2)-log(x))-log(1-(gamma.tp1.hat2/nu)^alpha))), ParamNames=c("alpha"),ParamEstimates=c(alpha.tp1.hat2))
  ...
  GtpSandwich <- SandwichMaker(simu$Value, simu$Weights, expression(w*(log(1/sigma)-((1+alpha)/alpha)*log((sigma+alpha*(x-gamma.gtp.hat1))/sigma) - log(1-(1+alpha*((nu-gamma.gtp.hat1)/sigma)^(-1/alpha))))), ParamNames=c("alpha", "sigma"),ParamEstimates=c(alpha.gtp.hat1,sigma.gtp.hat1))
